...
class cognex_insight_impl:
  ...
  config_Pose3X_Cell = "['C',520]"
  config_Pose3Y_Cell = "['D',520]"
  config_Pose3Theta_Cell = "['E',520]"
  ...
  def update(self):
    ...
    for k,v in res.iteritems():
      if v.has_key('x'):
        p = Pose()
        p.position.x = v['x']/1000.0
        p.position.y = v['y']/1000.0
        p.position.z = 0
        #q = quaternion_about_axis( math.radians(v['th']), (0, 0, 1))
        q = quaternion_from_euler(0,0,math.radians(-135.0 - v['th']))
        p.orientation.x = q[0]
        p.orientation.y = q[1]
        p.orientation.z = q[2]
        p.orientation.w = q[3]
